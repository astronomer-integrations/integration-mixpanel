...
Mixpanel.prototype.identify = function(identify, fn){
  ...
  var ignoreIp = identify.proxy('properties.ignoreIp')

  var ignoreTime = identify.proxy('properties.ignoreTime')
    || !identify.active();
...
Mixpanel.prototype._parseResponse = function(fn){
  ...
  return this.handle(function(err, res){
    ...
    if (!(res && res.body && res.body.status)) return fn(self.error(res && res.body && res.body.error));
    fn(null, res);
  });
};
